...
public class ConfigurationPropertyCollector {
    ...
    private static final Pattern SPI_PATTERN = Pattern.compile("(.*\\.spi$)|(.*\\.spi\\..*)|(.*\\.impl$)|(.*\\.impl\\..*)|(.*\\.internal$)|(.*\\.internal\\..*)");
    ...
    public ConfigurationPropertyCollector(String javadocsBaseLink, Path javadocsLocation, String anchor, String moduleName) {
        ...
        for (Element table : constants.select(".block-list li")) {
            ...
            if (!isClassIgnored(className)) {
                ...
                javadoc.ifPresent(doc -> {
                    Iterator<Element> div = table.selectFirst(".summary-table.three-column-summary").children().iterator();
                    // skip the header:
                    div.next();
                    div.next();
                    div.next();
                    ...
                    while (div.hasNext()) {
                        ...
                        if (!isConstantIgnored(className, constantText, value)) {
                            properties.put(className + "#" + constantText, new ConfigurationProperty().key(new ConfigurationProperty.Key(ConfigurationRules.prefixes(className, value), value)).javadoc(extractJavadoc(doc, className, constantText)).defaultValue(findDefault(constants, className, constantText)).sourceClass(className).anchorPrefix(anchor).moduleName(moduleName).type(extractType(className, constantText)));
                        }
                    }
                });
            }
    ...
    private Optional<Document> obtainJavadoc(String enclosingClass) {
        try {
            Path docs = javadocsLocation.resolve(enclosingClass.replace(".", File.separator) + ".html");
        ...
        if (!blocks.isEmpty()) {
            ...
            for (Element block : blocks) {
                ...
                if (block.hasClass("deprecation-block")) {
                    block.attr("style", "border-style:solid; border-width:thin;  border-radius:10px; padding:10px; margin-bottom:10px; margin-right:10px; display:inline-block;");
                }
    ...
    private String findDefault(Document constants, String className, String constant) {
        ...
        return constants.select(".caption").stream().filter(e -> e.text().equals(className + ".Defaults")).findAny().flatMap(// 2. if we found a table - select constants and find if the one we need is present:
        tableHeader -> tableHeader.siblingElements().select(".col-second").stream().filter(e -> e.text().equals(constant)).findAny().map(constantField -> constantField.nextElementSibling().text())).orElse(null);
    }
}
