...
public class ClientRest5ElasticsearchClient implements ElasticsearchClientImplementor {
    ...
    ClientRest5ElasticsearchClient(BeanHolder<? extends Rest5Client> restClientHolder, Sniffer sniffer, SimpleScheduledExecutor timeoutExecutorService, Optional<Integer> requestTimeoutMs, Gson gson, JsonLogHelper jsonLogHelper) {
    ...
    private ElasticsearchResponse convertResponse(Response response) {
        ...
        try {
            ...
            return new ElasticsearchResponse(response.getHost().toHostString(), response.getStatusCode(), reason, body);
        } catch (IOException | RuntimeException e) {
            throw ElasticsearchClientLog.INSTANCE.failedToParseElasticsearchResponse(response.getStatusCode(), reason, e.getMessage(), e);
        }
    }
