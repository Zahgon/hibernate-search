...
public abstract class AbstractConfiguredExtraPropertiesJsonAdapterFactory implements TypeAdapterFactory {
    ...
    private static class ReflectiveExtraPropertyAdapter<T> implements ExtraPropertyAdapter<T> {
        ...
        private void setExtraProperties(T instance, Map<String, JsonElement> extraProperties) {
            try {
                ...
                throw new AssertionFailure("Field " + extraPropertiesField + " annotated with " + SerializeExtraProperties.class + " has the wrong type on " + instance.getClass());
            } catch (IllegalAccessException e) {
                throw new AssertionFailure("Field " + extraPropertiesField + " annotated with " + SerializeExtraProperties.class + " is inaccessible on " + instance.getClass());
            }
        }
    ...
    private static class Adapter<T> extends AbstractExtraPropertiesJsonAdapter<T> {
        ...
        public Adapter(Map<String, ? extends FieldAdapter<? super T>> fieldAdapters, ExtraPropertyAdapter<? super T> extraPropertyAdapter, Constructor<T> constructor) {
