...
public final class ElasticsearchResponseFormatter {
    ...
    private static String formatResponse(ElasticsearchResponse response) {
        ...
        sb.append(response.statusCode()).append(" '").append(response.statusMessage()).append("' from '").append(response.hostAndPort()).append("' with body ").append(helper.toString(response.body()));
        return sb.toString();
    }
}
