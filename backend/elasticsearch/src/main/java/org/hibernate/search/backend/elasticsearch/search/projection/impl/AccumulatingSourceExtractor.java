...
abstract class AccumulatingSourceExtractor<E, V, A, P> implements ElasticsearchSearchProjection.Extractor<A, P> {
    ...
    public AccumulatingSourceExtractor(String[] fieldPathComponents, ProjectionCollector<E, V, A, P> collector) {
        ...
        if (currentPathComponentIndex == (fieldPathComponents.length - 1)) {
            ...
            if (child == null || child.isJsonNull()) {
                ...
                for (JsonElement childElement : child.getAsJsonArray()) {
                    ...
                    accumulated = collect(projectionHitMapper, hit, childElementAsObject, context, accumulated, currentPathComponentIndex + 1);
                }
                return accumulated;
            } else {
                JsonObject childAsObject = toJsonObject(child, currentPathComponentIndex);
                return collect(projectionHitMapper, hit, childAsObject, context, accumulated, currentPathComponentIndex + 1);
            }
        }
    }
    ...
    private A collectTargetField(ProjectionHitMapper<?> projectionHitMapper, JsonObject hit, JsonElement fieldValue, ProjectionExtractContext context, A accumulated) {
        if (fieldValue == null) {
            ...
            for (JsonElement childElement : fieldValue.getAsJsonArray()) {
                accumulated = collector.accumulate(accumulated, extract(projectionHitMapper, hit, childElement, context));
            }
            return accumulated;
        } else {
            return collector.accumulate(accumulated, extract(projectionHitMapper, hit, fieldValue, context));
    ...
    protected abstract E extract(ProjectionHitMapper<?> projectionHitMapper, JsonObject hit, JsonElement sourceElement, ProjectionExtractContext context);
    ...
    private JsonObject toJsonObject(JsonElement childElement, int currentPathComponentIndex) {
        ...
        if (!JsonElementTypes.OBJECT.isInstance(childElement)) {
            throw new UnexpectedJsonElementTypeException(Arrays.stream(fieldPathComponents, 0, currentPathComponentIndex + 1).collect(Collectors.joining(".")), JsonElementTypes.OBJECT, childElement);
        }
