...
public class ElasticsearchRequestSuccessAssessor {
    ...
    private static final JsonAccessor<Integer> FAILED_SHARDS_COUNT = JsonAccessor.root().property("_shards").property("failed").asInteger();

    private static final int TIME_OUT_HTTP_STATUS_CODE = 408;

    public static final ElasticsearchRequestSuccessAssessor SHARD_FAILURE_CHECKED_INSTANCE = builder().ignoreShardFailures(false).build();
    ...
    private boolean isSuccess(Optional<Integer> statusCode, JsonObject responseBody) {
        return statusCode.map(c -> ElasticsearchClientUtils.isSuccessCode(c) || ignoredErrorStatuses.contains(c)).orElse(false) && (FAILED_SHARDS_COUNT.get(responseBody).map(this::checkShardFailures).orElse(true)) || ERROR_TYPE.get(responseBody).map(ignoredErrorTypes::contains).orElse(false);
    }
