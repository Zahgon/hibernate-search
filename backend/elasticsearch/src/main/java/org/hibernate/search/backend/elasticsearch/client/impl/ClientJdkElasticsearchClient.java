...
public class ClientJdkElasticsearchClient implements ElasticsearchClientImplementor {
    ...
    ClientJdkElasticsearchClient(BeanHolder<? extends RestJdkClient> restClientHolder, SimpleScheduledExecutor timeoutExecutorService, Optional<Integer> requestTimeoutMs, Gson gson, JsonLogHelper jsonLogHelper, List<HttpRequestInterceptor> requestInterceptors) {
    ...
    private ElasticsearchResponse convertResponse(HttpResponse<JsonObject> response) {
        try {
            return new ElasticsearchResponse(response.request().uri().getHost(), response.statusCode(), null, response.body());
        } catch (RuntimeException e) {
            throw ElasticsearchClientLog.INSTANCE.failedToParseElasticsearchResponse(response.statusCode(), null, e.getMessage(), e);
        }
    }
    ...
    private CompletableFuture<HttpResponse<JsonObject>> send(ElasticsearchRequest elasticsearchRequest) {
        ...
        CompletableFuture<HttpResponse<JsonObject>> completableFuture = restClientHolder.get().sendAsync(request, new JsonObjectBodyHandler());
        ...
        long currentTimeoutValue = deadline == null ? Long.valueOf(requestTimeoutMs.get()) : deadline.checkRemainingTimeMillis();
        /*
        ...
        ScheduledFuture<?> timeout = timeoutExecutorService.schedule(() -> {
            if (!completableFuture.isDone()) {
                RuntimeException cause = ElasticsearchClientLog.INSTANCE.requestTimedOut(Duration.ofNanos(TimeUnit.MILLISECONDS.toNanos(currentTimeoutValue)), elasticsearchRequest);
                completableFuture.completeExceptionally(deadline != null ? deadline.forceTimeoutAndCreateException(cause) : cause);
            }
    ...
    private HttpRequest toRequest(ElasticsearchRequest elasticsearchRequest, HttpRequest.BodyPublisher bodyPublisher) throws IOException {
        URI uri = toUri(elasticsearchRequest);
        HttpRequest.Builder request = HttpRequest.newBuilder(uri).method(elasticsearchRequest.method(), bodyPublisher);
    ...
    private URI toUri(ElasticsearchRequest elasticsearchRequest) {
        return restClientHolder.get().nextNode().createRequestURI(elasticsearchRequest.path(), elasticsearchRequest.parameters());
    }
    ...
    private void log(ElasticsearchRequest request, long start, ElasticsearchResponse response) {
        ...
        if (successCode) {
            ElasticsearchRequestLog.INSTANCE.executedRequest(request.method(), response.hostAndPort(), request.path(), request.parameters(), request.bodyParts().size(), executionTimeMs, response.statusCode(), response.statusMessage(), jsonLogHelper.toString(request.bodyParts()), jsonLogHelper.toString(response.body()));
        } else {
            ElasticsearchRequestLog.INSTANCE.executedRequestWithFailure(request.method(), response.hostAndPort(), request.path(), request.parameters(), request.bodyParts().size(), executionTimeMs, response.statusCode(), response.statusMessage(), jsonLogHelper.toString(request.bodyParts()), jsonLogHelper.toString(response.body()));
        }
    }
