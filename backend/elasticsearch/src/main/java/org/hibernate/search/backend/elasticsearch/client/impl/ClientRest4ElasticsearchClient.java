...
public class ClientRest4ElasticsearchClient implements ElasticsearchClientImplementor {
    ...
    ClientRest4ElasticsearchClient(BeanHolder<? extends RestClient> restClientHolder, Sniffer sniffer, SimpleScheduledExecutor timeoutExecutorService, Optional<Integer> requestTimeoutMs, int connectionTimeoutMs, Gson gson, JsonLogHelper jsonLogHelper) {
    ...
    private CompletableFuture<Response> send(ElasticsearchRequest elasticsearchRequest) {
        ...
        restClientHolder.get().performRequestAsync(toRequest(elasticsearchRequest, entity), new ResponseListener() {
    ...
    private void setPerRequestSocketTimeout(ElasticsearchRequest elasticsearchRequest, Request request) {
        ...
        RequestConfig requestConfig = RequestConfig.custom().setConnectionRequestTimeout(//Disable lease handling for the connection pool! See also HSEARCH-2681
        0).setSocketTimeout(socketTimeoutMs).setConnectTimeout(connectionTimeoutMs).build();
    ...
    private ElasticsearchResponse convertResponse(Response response) {
        try {
            ...
            return new ElasticsearchResponse(response.getHost().toHostString(), response.getStatusLine().getStatusCode(), response.getStatusLine().getReasonPhrase(), body);
        } catch (IOException | RuntimeException e) {
            throw ElasticsearchClientLog.INSTANCE.failedToParseElasticsearchResponse(response.getStatusLine().getStatusCode(), response.getStatusLine().getReasonPhrase(), e.getMessage(), e);
        }
    }
