...
public class ElasticsearchDistanceToFieldProjection<A, P> extends AbstractElasticsearchProjection<P> implements ElasticsearchSearchProjection.Extractor<A, P> {
    ...
    private static final String DISTANCE_PROJECTION_SCRIPT = // Check whether the field exists first with "containsKey";
    ...
    " if (doc.containsKey(params.fieldPath) && doc[params.fieldPath].size() != 0) {" + " return doc[params.fieldPath].arcDistance(params.lat, params.lon);" + " } else {" + " return null;" + " }";
    ...
    private ElasticsearchDistanceToFieldProjection(Builder builder, ProjectionCollector.Provider<Double, P> collectorProvider, ProjectionCollector<Double, Double, A, P> collector) {
        super(builder);
        ...
        if (singleValuedInRoot && builder.field.nestedPathHierarchy().isEmpty()) {
            ...
            this.sourceProjection = new ElasticsearchFieldProjection<>(builder.scope, builder.field, this::computeDistanceWithUnit, false, NO_OP_DOUBLE_CONVERTER, collectorProvider);
        }
    }
