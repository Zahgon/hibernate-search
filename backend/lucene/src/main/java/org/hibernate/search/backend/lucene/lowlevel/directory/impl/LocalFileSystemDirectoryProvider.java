...
public class LocalFileSystemDirectoryProvider implements DirectoryProvider {
    ...
    private static final ConfigurationProperty<Path> ROOT = ConfigurationProperty.forKey(LuceneIndexSettings.DirectoryRadicals.ROOT).as(Path.class, Paths::get).withDefault(() -> Paths.get(LuceneIndexSettings.Defaults.DIRECTORY_ROOT)).build();
