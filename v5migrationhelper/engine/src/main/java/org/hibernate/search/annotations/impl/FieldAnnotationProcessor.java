...
public class FieldAnnotationProcessor implements PropertyMappingAnnotationProcessor<Field> {
    ...
    private PropertyMappingGenericFieldOptionsStep mapGeneric(PropertyMappingStep mapping, Field annotation, PropertyMappingAnnotationProcessorContext context) {
        ...
        PropertyMappingGenericFieldOptionsStep genericOptionsStep = mapping.genericField(name).sortable(sortable(name, context) ? Sortable.YES : Sortable.NO);
        ...
        for (String facetName : facetNames) {
            if (facetName.isEmpty() || facetName.equals(name)) {
                ...
                PropertyMappingGenericFieldOptionsStep facetOptionsStep = mapping.genericField(facetName).aggregable(Aggregable.YES).searchable(Searchable.NO);
                if (valueBridge != null) {
    ...
    private PropertyMappingStandardFieldOptionsStep<?> mapFullText(PropertyMappingStep mapping, Field annotation, PropertyMappingAnnotationProcessorContext context, String analyzerOrDefault) {
        String name = name(annotation, context);
        PropertyMappingFullTextFieldOptionsStep fullTextOptionsStep = mapping.fullTextField(name).analyzer(analyzerOrDefault).norms(norms(annotation)).termVector(termVector(annotation));
    ...
    private PropertyMappingKeywordFieldOptionsStep mapKeyword(PropertyMappingStep mapping, Field annotation, PropertyMappingAnnotationProcessorContext context, String normalizer) {
        String name = name(annotation, context);
        PropertyMappingKeywordFieldOptionsStep keywordOptionsStep = mapping.keywordField(name).norms(norms(annotation)).sortable(sortable(name, context) ? Sortable.YES : Sortable.NO);
        if (normalizer != null) {
        ...
        for (String facetName : facetNames) {
            if (facetName.isEmpty() || facetName.equals(name)) {
                ...
                PropertyMappingKeywordFieldOptionsStep facetOptionsStep = mapping.keywordField(facetName).aggregable(Aggregable.YES).searchable(Searchable.NO);
                if (normalizer != null) {
    ...
    private TermVector termVector(Field annotation) {
        ...
        switch(value) {
            case YES:
                // This is the Search 6 TermVector, a different type.
    ...
    private BeanReference<? extends ValueBridge<?, ?>> valueBridge(PropertyMappingAnnotationProcessorContext context) {
        MappingAnnotatedProperty annotatedProperty = context.annotatedElement();
        return annotatedProperty.allAnnotations().map(a -> valueBridge(annotatedProperty, a)).filter(Objects::nonNull).findAny().orElse(null);
    }
    ...
    private BeanReference<? extends ValueBridge<?, ?>> valueBridge(MappingAnnotatedProperty annotatedProperty, Annotation annotation) {
        ...
        } else if (Calendar.class.isAssignableFrom(annotatedProperty.javaClass()) && annotation.annotationType().equals(CalendarBridge.class)) {
