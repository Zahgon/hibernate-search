...
public record EffectiveRange(long min, long max) {
    ...
    private static <T> EffectiveRange[] createEffectiveRangesForIntegralValues(Collection<? extends Range<? extends T>> ranges, ToLongFunction<T> encoder, T lowestPossibleValue, T highestPossibleValue, boolean extremaAreInfinity) {
        ...
        for (Range<? extends T> range : ranges) {
            ...
            if (RangeBoundInclusion.EXCLUDED.equals(range.lowerBoundInclusion()) && (extremaAreInfinity || lowerBoundValue != null)) {
                ++min;
            }
            // The upper bound is included if it is explicitly included
            // ... or if it is infinity but infinity cannot be represented
            // so if it's none of the above we exclude the boundary by -- it.
            if (RangeBoundInclusion.EXCLUDED.equals(range.upperBoundInclusion()) && (extremaAreInfinity || upperBoundValue != null)) {
                --max;
            }
