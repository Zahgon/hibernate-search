...
class LuceneSearcherImpl<H> implements LuceneSearcher<LuceneLoadableSearchResult<H>, LuceneExtractableSearchResult<H>> {
    ...
    LuceneSearcherImpl(LuceneSearchQueryRequestContext requestContext, LuceneSearchProjection.Extractor<?, H> rootExtractor, Map<AggregationKey<?>, LuceneSearchAggregation.Extractor<?>> aggregations, ExtractionRequirements extractionRequirements, TimeoutManager timeoutManager) {
    ...
    @Override
    public LuceneExtractableSearchResult<H> scroll(IndexSearcher indexSearcher, IndexReaderMetadataResolver metadataResolver, int offset, int limit, int totalHitCountThreshold) throws IOException {
        throw new UnsupportedOperationException("STUB: not implemented");
    }

    private LuceneExtractableSearchResult<H> doSearch(IndexSearcher indexSearcher, IndexReaderMetadataResolver metadataResolver, int offset, Integer limit, int totalHitCountThreshold) throws IOException {
        ...
        LuceneCollectors luceneCollectors = (limit != null || maxDocs <= PREFETCH_HITS_SIZE) ? collectMatchingDocs(indexSearcher, metadataResolver, offset, maxDocs, totalHitCountThreshold) : collectMatchingDocsWithPrefetch(indexSearcher, metadataResolver, offset, maxDocs, totalHitCountThreshold);
        return new LuceneExtractableSearchResult<>(requestContext, indexSearcher, luceneCollectors, rootExtractor, aggregations, timeoutManager);
    }
    ...
    private LuceneCollectors collectMatchingDocs(IndexSearcher indexSearcher, IndexReaderMetadataResolver metadataResolver, int offset, int maxDocs, int totalHitCountThreshold) throws IOException {
        LuceneCollectors luceneCollectors = buildCollectors(indexSearcher, metadataResolver, maxDocs, offset, totalHitCountThreshold);
        luceneCollectors.collectMatchingDocs();
        return luceneCollectors;
    }

    private LuceneCollectors collectMatchingDocsWithPrefetch(IndexSearcher indexSearcher, IndexReaderMetadataResolver metadataResolver, int offset, int maxDocs, int totalHitCountThreshold) throws IOException {
        // prefetch:
        LuceneCollectors luceneCollectors = collectMatchingDocs(indexSearcher, metadataResolver, offset, PREFETCH_HITS_SIZE, Math.max(totalHitCountThreshold, PREFETCH_TOTAL_HIT_COUNT_THRESHOLD));
    ...
    private LuceneCollectors buildCollectors(IndexSearcher indexSearcher, IndexReaderMetadataResolver metadataResolver, int maxDocs, int offset, int totalHitCountThreshold) throws IOException {
        return extractionRequirements.createCollectors(indexSearcher, requestContext.getLuceneQuery(), requestContext.getLuceneSort(), metadataResolver, maxDocs, offset, timeoutManager, totalHitCountThreshold);
    }
