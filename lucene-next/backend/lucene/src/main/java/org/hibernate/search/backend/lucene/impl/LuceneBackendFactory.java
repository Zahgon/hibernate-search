...
public class LuceneBackendFactory implements BackendFactory {
    ...
    private static final OptionalConfigurationProperty<MultiTenancyStrategyName> MULTI_TENANCY_STRATEGY = ConfigurationProperty.forKey(LuceneBackendSettings.MULTI_TENANCY_STRATEGY).as(MultiTenancyStrategyName.class, MultiTenancyStrategyName::of).build();

    private static final OptionalConfigurationProperty<List<BeanReference<? extends LuceneAnalysisConfigurer>>> ANALYSIS_CONFIGURER = ConfigurationProperty.forKey(LuceneBackendSettings.ANALYSIS_CONFIGURER).asBeanReference(LuceneAnalysisConfigurer.class).multivalued().build();

    private static final OptionalConfigurationProperty<List<BeanReference<? extends QueryCachingConfigurer>>> QUERY_CACHING_CONFIGURER = ConfigurationProperty.forKey(LuceneBackendSettings.QUERY_CACHING_CONFIGURER).asBeanReference(QueryCachingConfigurer.class).multivalued().build();
    ...
    private Version getLuceneVersion(EventContext backendContext, ConfigurationPropertySource propertySource) {
        ...
        if (luceneVersionOptional.isPresent()) {
            ...
            ConfigurationLog.INSTANCE.recommendConfiguringLuceneVersion(LUCENE_VERSION.resolveOrRaw(propertySource), latestVersion, backendContext);
    ...
    private LuceneAnalysisDefinitionRegistry getAnalysisDefinitionRegistry(BackendBuildContext buildContext, ConfigurationPropertySource propertySource, Version luceneVersion) {
        try {
            ...
            ANALYSIS_CONFIGURER.getAndMap(propertySource, beanResolver::resolve).ifPresent(holder -> {
    ...
    private void configureQueryCache(BackendBuildContext buildContext, ConfigurationPropertySource propertySource, QueryCachingConfigurationContext context) {
        try {
            final BeanResolver beanResolver = buildContext.beanResolver();
            try (BeanHolder<List<QueryCachingConfigurer>> implicitConfigurersHolder = beanResolver.resolve(beanResolver.allConfiguredForRole(QueryCachingConfigurer.class))) {
            ...
            QUERY_CACHING_CONFIGURER.getAndMap(propertySource, beanResolver::resolve).ifPresent(holder -> {
