...
class ExplainWork implements ReadWork<Explanation> {
    ...
    private int getLuceneDocId(ReadWorkExecutionContext context, IndexSearcher indexSearcher) throws IOException {
        ...
        if (topDocs.scoreDocs.length > 1) {
            throw new AssertionFailure("Multiple documents match query " + explainedDocumentQuery + ".");
        }
        return topDocs.scoreDocs[0].doc;
    }

    private Query createExplainedDocumentQuery(ReadWorkExecutionContext context) {
        BooleanQuery.Builder builder = new BooleanQuery.Builder().add(Queries.mainDocumentQuery(), BooleanClause.Occur.FILTER).add(Queries.term(MetadataFields.idFieldName(), explainedDocumentId), BooleanClause.Occur.FILTER).add(new MappedTypeNameQuery(context.getIndexReaderMetadataResolver(), explainedDocumentTypeName), BooleanClause.Occur.FILTER);
