...
public class LuceneCollectors {
    ...
    private void processCollectedMatchingDocs() throws IOException {
        ...
        if (resultTotal == null) {
            boolean exact = TotalHits.Relation.EQUAL_TO.equals(topDocs.totalHits.relation()) && !timeoutManager.isTimedOut();
    ...
    private <T> HibernateSearchMultiCollectorManager buildTopDocsDataCollectors(TopDocsDataCollector.Factory<T> collectorManagerFactory) throws IOException {
        TopDocsDataCollectorExecutionContext executionContext = new TopDocsDataCollectorExecutionContext(metadataResolver, indexSearcher, rewrittenLuceneQuery, originalLuceneQuery, topDocs, storedFieldsValuesDelegateOrNull);
        HibernateSearchMultiCollectorManager.Builder collectorForTopDocsBuilder = new HibernateSearchMultiCollectorManager.Builder(executionContext, timeoutManager);
