...
public class TreeFilter {
    ...
    private boolean isPathIncludedInternal(int relativeDepth, String relativePath, boolean markIncludedAsEncountered) {
        boolean includedByThis = (depthFilter.isEveryPathIncludedAtDepth(relativeDepth) || pathFilter.isExplicitlyIncluded(relativePath)) && !pathFilter.isExplicitlyExcluded(relativePath);
        ...
        if (includedByThis && parent != null) {
            includedByParent = parent.isPathIncludedInternal(relativeDepth + 1, relativePrefix + relativePath, markIncludedAsEncountered);
        }
    ...
    private boolean isAnyPathExplicitlyIncluded(String prefixToRemove, TreeFilter filter) {
		 ...
		 */
        return parent != null && parent.isAnyPathExplicitlyIncluded(this.relativePrefix + prefixToRemove, filter);
    }

    private boolean isEveryPathIncludedAtDepth(int depth) {
        return depthFilter.isEveryPathIncludedAtDepth(depth) && // The parent can reduce the max depth of a child.
    ...
    private boolean isSame(MappingElement mappingElement, String relativePrefix, TreeFilterDefinition definition) {
        return // no need to check the root filter any further
        this.mappingElement != null && this.mappingElement.equals(mappingElement) && // Technically we shouldn't have to check these,
        ...
        this.relativePrefix.equals(relativePrefix) && this.definition.equals(definition);
    }
    ...
    private boolean hasCompositionLimits() {
        return depthFilter.hasDepthLimit() || pathFilter.isAnyPathExplicitlyIncluded() || parent != null && parent.hasCompositionLimits();
    }
}
