...
public final class ProcessorElementUtils {
        ...
        for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> entry : annotationMirror.getElementValues().entrySet()) {
            if (entry.getKey().getSimpleName().contentEquals("value")) {
                if (entry.getKey().getReturnType() instanceof ArrayType arrayType) {
                    Optional<? extends AnnotationMirror> repeatable = types.asElement(arrayType.getComponentType()).getAnnotationMirrors().stream().filter(a -> ((TypeElement) a.getAnnotationType().asElement()).getQualifiedName().contentEquals("java.lang.annotation.Repeatable")).findAny();
                    if (repeatable.isPresent()) {
                        TypeMirror returnType = (TypeMirror) repeatable.get().getElementValues().entrySet().iterator().next().getValue().getValue();
    ...
    private static boolean isGetter(ExecutableElement executable) {
        ...
        if (returnType.getKind() == TypeKind.BOOLEAN || !returnType.getKind().isPrimitive() && returnType.toString().equals("java.lang.Boolean")) {
