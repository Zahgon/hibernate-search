...
public class PojoScopeSchemaManagerImpl implements PojoScopeSchemaManager {
    ...
    private CompletableFuture<?> doOperationOnTypesBiFunction(BiFunction<IndexSchemaManager, OperationSubmitter, CompletableFuture<?>> operation, FailureCollector failureCollector, OperationSubmitter operationSubmitter) {
        return doOperationOnTypesTriFunction((schemaManager, typeFailureCollector, submitter) -> operation.apply(schemaManager, submitter), failureCollector, operationSubmitter);
    }
    ...
    private CompletableFuture<?> doOperationOnTypesTriFunction(TriFunction<IndexSchemaManager, ContextualFailureCollector, OperationSubmitter, CompletableFuture<?>> operation, FailureCollector failureCollector, OperationSubmitter operationSubmitter) {
        ...
        for (PojoSchemaManagementIndexedTypeContext typeContext : targetedTypeContexts) {
            ...
            ContextualFailureCollector typeFailureCollector = failureCollector.withContext(PojoEventContexts.fromType(typeContext.typeIdentifier()));
            futures[typeCounter++] = operation.apply(delegate, typeFailureCollector, operationSubmitter).exceptionally(Futures.handler(e -> {
