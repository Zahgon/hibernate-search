...
public class ProjectionBindingContextImpl<P> implements ProjectionBindingContext {
    ...
    private static final BiFunction<MappingElement, String, SearchException> CYCLIC_RECURSION_EXCEPTION_FACTORY = (mappingElement, cyclicRecursionPath) -> ProjectionLog.INSTANCE.objectProjectionCyclicRecursion(mappingElement, mappingElement.eventContext(), cyclicRecursionPath);

    private static final Set<String> CONTAINER_EXTRACTORS = Set.of(BuiltinContainerExtractors.ARRAY_OBJECT, BuiltinContainerExtractors.ARRAY_CHAR, BuiltinContainerExtractors.ARRAY_BOOLEAN, BuiltinContainerExtractors.ARRAY_BYTE, BuiltinContainerExtractors.ARRAY_SHORT, BuiltinContainerExtractors.ARRAY_INT, BuiltinContainerExtractors.ARRAY_LONG, BuiltinContainerExtractors.ARRAY_FLOAT, BuiltinContainerExtractors.ARRAY_DOUBLE, BuiltinContainerExtractors.COLLECTION, BuiltinContainerExtractors.ITERABLE, // and some single ones:
    BuiltinContainerExtractors.OPTIONAL, BuiltinContainerExtractors.OPTIONAL_DOUBLE, BuiltinContainerExtractors.OPTIONAL_INT, BuiltinContainerExtractors.OPTIONAL_LONG);

    private static final Set<String> MULTI_EXTRACTORS = Set.of(BuiltinContainerExtractors.COLLECTION, BuiltinContainerExtractors.ITERABLE);
    ...
    private PojoTypeModel<?> boundParameterElement(Set<String> extractors) {
        BoundContainerExtractorPath<?, ?> boundParameterElementPath = mappingHelper.indexModelBinder().bindExtractorPath(parameterTypeModel, ContainerExtractorPath.defaultExtractors());
    ...
    private <T> Optional<T> nestObjectProjection(String fieldPath, TreeFilterDefinition filter, Function<TreeNestingContext, T> contextBuilder) {
        return parameterBinder.parent.nestingContext.nestComposed(mappingElement, fieldPath + ".", filter, mappingHelper.getOrCreatePathTracker(mappingElement, filter), new TreeNestingContext.NestedContextBuilder<T>() {
    ...
    private <T> CompositeProjectionDefinition<T> createCompositeProjectionDefinition(Class<T> projectedType, TreeNestingContext nestingContext) {
        PojoConstructorModel<T> projectionConstructor = parameterBinder.findProjectionConstructorOrNull(mappingHelper.introspector().typeModel(projectedType));
        if (projectionConstructor == null) {
            throw ProjectionLog.INSTANCE.invalidObjectClassForProjection(projectedType);
        }
        return new ProjectionConstructorBinder<>(mappingHelper, projectionConstructor, this, nestingContext).bind();
    }
    ...
    private <P2, C> void checkAndBind(BeanHolder<? extends ProjectionDefinition<? extends C>> definitionHolder, PojoRawTypeModel<P2> expectedValueType) {
        ...
        if (containerElementModel.isPresent()) {
            if (!expectedValueType.isSubTypeOf(containerElementModel.get().rawType())) {
                throw ProjectionLog.INSTANCE.invalidOutputTypeForMultiValuedProjectionDefinition(definitionHolder.get(), parameterTypeModel, expectedValueType);
            }
        } else if (!expectedValueType.isSubTypeOf(parameterTypeModel.rawType())) {
            throw ProjectionLog.INSTANCE.invalidOutputTypeForProjectionDefinition(definitionHolder.get(), parameterTypeModel, expectedValueType);
        }
        // We check that P2 extends P explicitly using reflection (see above)
        @SuppressWarnings("unchecked")
        BeanHolder<? extends ProjectionDefinition<? extends P>> castDefinitionHolder = (BeanHolder<? extends ProjectionDefinition<? extends P>>) definitionHolder;
    ...
    public class MultiContextImpl<PV> implements ProjectionBindingMultiContext {
        ...
        public MultiContextImpl(PojoTypeModel<PV> parameterContainerElementTypeModel) {
            ...
            this.parameterContainerElementRootElement = new PojoModelValueElement<>(mappingHelper.introspector(), parameterContainerElementTypeModel);
        }
