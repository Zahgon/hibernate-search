...
public class PojoMassIndexingEntityLoadingRunnable<E, I> extends PojoMassIndexingFailureHandledRunnable {
    ...
    private final class IndexingBatch {
        ...
        private void waitForIndexingEndAndReport() throws InterruptedException {
            ...
            Futures.unwrappedExceptionGet(CompletableFuture.allOf(indexingFutures).// We handle exceptions on a per-entity basis below, so we ignore them here.
            ...
            for (int i = 0; i < entities.size(); i++) {
                ...
                if (future.isCompletedExceptionally()) {
                    ...
                    getNotifier().reportEntityIndexingFailure(// We don't try to detect the exact entity type here,
                    ...
                    typeGroup, sessionContext, entity, Throwables.expectException(Futures.getThrowableNow(future)));
                } else {
                    ++successfulEntities;
                }
            }
        ...
        private CompletableFuture<?> startIndexing(PojoMassIndexingSessionContext sessionContext, PojoIndexer indexer, Object entity) throws InterruptedException {
            ...
            try {
                ...
                future = indexer.add(typeIdentifier, null, null, entity, // Commit and refresh are handled globally after all documents are indexed.
