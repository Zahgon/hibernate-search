...
class AnnotationPojoTypeMetadataContributorFactory {
    ...
    private boolean processAnnotations(TypeMappingStepImpl typeMappingContext, PojoRawTypeModel<?> typeModel) {
        ...
        List<Annotation> annotationList = typeModel.annotations().flatMap(annotationHelper::expandRepeatableContainingAnnotation).collect(Collectors.toList());
    ...
    private boolean processConstructorLevelAnnotations(ConstructorMappingStep mappingContext, PojoRawTypeModel<?> typeModel, PojoConstructorModel<?> constructorModel) {
        boolean processedAtLeastOneAnnotation = false;
        List<Annotation> annotationList = constructorModel.annotations().flatMap(annotationHelper::expandRepeatableContainingAnnotation).collect(Collectors.toList());
        for (Annotation annotation : annotationList) {
    ...
    private boolean processMethodParameterLevelAnnotations(MethodParameterMappingStep mappingContext, PojoConstructorModel<?> constructorModel, PojoMethodParameterModel<?> methodParameterModel) {
        boolean processedAtLeastOneAnnotation = false;
        List<Annotation> annotationList = methodParameterModel.annotations().flatMap(annotationHelper::expandRepeatableContainingAnnotation).collect(Collectors.toList());
        for (Annotation annotation : annotationList) {
    ...
    private boolean processPropertyLevelAnnotations(PropertyMappingStep mappingContext, PojoRawTypeModel<?> typeModel, PojoPropertyModel<?> propertyModel) {
        boolean processedAtLeastOneAnnotation = false;
        List<Annotation> annotationList = propertyModel.annotations().flatMap(annotationHelper::expandRepeatableContainingAnnotation).collect(Collectors.toList());
        for (Annotation annotation : annotationList) {
    ...
    private <A extends Annotation> boolean tryApplyProcessor(TypeMappingStep mapping, PojoRawTypeModel<?> typeModel, A annotation) {
        Optional<BeanHolder<? extends TypeMappingAnnotationProcessor<? super A>>> processorOptional = annotationProcessorProvider.createTypeAnnotationProcessor(annotation);
        if (!processorOptional.isPresent()) {
            return false;
        }
        TypeMappingAnnotationProcessorContext context = new TypeMappingAnnotationProcessorContextImpl(typeModel, annotation, annotationHelper);
        try (BeanHolder<? extends TypeMappingAnnotationProcessor<? super A>> processorHolder = processorOptional.get()) {
    ...
    private <A extends Annotation> boolean tryApplyProcessor(ConstructorMappingStep mapping, PojoRawTypeModel<?> typeModel, PojoConstructorModel<?> constructorModel, A annotation) {
        Optional<BeanHolder<? extends ConstructorMappingAnnotationProcessor<? super A>>> processorOptional = annotationProcessorProvider.createConstructorAnnotationProcessor(annotation);
        if (!processorOptional.isPresent()) {
            return false;
        }
        ConstructorMappingAnnotationProcessorContext context = new ConstructorMappingAnnotationProcessorContextImpl(typeModel, constructorModel, annotation, annotationHelper);
        try (BeanHolder<? extends ConstructorMappingAnnotationProcessor<? super A>> processorHolder = processorOptional.get()) {
            processorHolder.get().process(mapping, annotation, context);
        } catch (RuntimeException e) {
            rootFailureCollector.withContext(context.eventContext()).add(e);
        }
    ...
    private <A extends Annotation> boolean tryApplyProcessor(MethodParameterMappingStep mapping, PojoConstructorModel<?> constructorModel, PojoMethodParameterModel<?> methodParameterModel, A annotation) {
        Optional<BeanHolder<? extends MethodParameterMappingAnnotationProcessor<? super A>>> processorOptional = annotationProcessorProvider.createMethodParameterAnnotationProcessor(annotation);
        if (!processorOptional.isPresent()) {
            return false;
        }
        MethodParameterMappingAnnotationProcessorContext context = new MethodParameterMappingAnnotationProcessorContextImpl(constructorModel, methodParameterModel, annotation, annotationHelper);
        try (BeanHolder<? extends MethodParameterMappingAnnotationProcessor<? super A>> processorHolder = processorOptional.get()) {
            processorHolder.get().process(mapping, annotation, context);
        } catch (RuntimeException e) {
            rootFailureCollector.withContext(context.eventContext()).add(e);
        }
    ...
    private <A extends Annotation> boolean tryApplyProcessor(PropertyMappingStep mapping, PojoRawTypeModel<?> typeModel, PojoPropertyModel<?> propertyModel, A annotation) {
        Optional<BeanHolder<? extends PropertyMappingAnnotationProcessor<? super A>>> processorOptional = annotationProcessorProvider.createPropertyAnnotationProcessor(annotation);
        if (!processorOptional.isPresent()) {
            return false;
        }
        PropertyMappingAnnotationProcessorContext context = new PropertyMappingAnnotationProcessorContextImpl(typeModel, propertyModel, annotation, annotationHelper, configurationContext);
        try (BeanHolder<? extends PropertyMappingAnnotationProcessor<? super A>> processorHolder = processorOptional.get()) {
            processorHolder.get().process(mapping, annotation, context);
        } catch (RuntimeException e) {
            rootFailureCollector.withContext(context.eventContext()).add(e);
        }
