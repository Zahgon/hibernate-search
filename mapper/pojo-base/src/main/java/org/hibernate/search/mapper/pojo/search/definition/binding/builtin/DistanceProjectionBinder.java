...
public final class DistanceProjectionBinder implements ProjectionBinder {
    ...
    private void bind(ProjectionBindingContext context, String fieldPath, ProjectionCollector.Provider<Double, ?> collector) {
        context.definition(Double.class, context.isIncluded(fieldPath) ? BeanHolder.of(new DistanceProjectionDefinition.WrappedValued<>(fieldPath, parameterName, unit, collector)) : ConstantProjectionDefinition.empty(collector));
    }
