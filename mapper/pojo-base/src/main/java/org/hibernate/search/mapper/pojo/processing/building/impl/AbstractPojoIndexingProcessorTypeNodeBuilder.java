 ...
 */
public abstract class AbstractPojoIndexingProcessorTypeNodeBuilder<T, U> extends AbstractPojoProcessorNodeBuilder implements PojoIndexMappingCollectorTypeNode {
    ...
    private PojoIndexingProcessorPropertyNodeBuilder<U, ?> createPropertyNodeBuilder(String propertyName) {
        return new PojoIndexingProcessorPropertyNodeBuilder<>(getModelPath().property(propertyName), mappingHelper, bindingContext, identityMappingCollector);
    }
    ...
    private Optional<PojoIndexingProcessor<T>> doBuild(PojoIndexingDependencyCollectorTypeNode<U> dependencyCollector) {
        ...
        try {
            ...
            propertyNodeBuilders.values().stream().map(builder -> builder.build(dependencyCollector)).filter(Optional::isPresent).map(Optional::get).forEach(nestedNodes::add);
            if (parentIndexObjectReferences.isEmpty() && nestedNodes.isEmpty()) {
                ...
                return Optional.of(doBuild(parentIndexObjectReferences, createNested(nestedNodes)));
            }
