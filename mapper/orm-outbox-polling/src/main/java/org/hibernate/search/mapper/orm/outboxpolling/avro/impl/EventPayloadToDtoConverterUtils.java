...
final class EventPayloadToDtoConverterUtils {
    ...
    private static DirtinessDescriptorDto convert(DirtinessDescriptor dirtiness) {
        return DirtinessDescriptorDto.newBuilder().setDirtyPaths(convertDirtyPaths(dirtiness.dirtyPaths())).setForceContainingDirty(dirtiness.forceContainingDirty()).setForceSelfDirty(dirtiness.forceSelfDirty()).setUpdateBecauseOfContained(dirtiness.updatedBecauseOfContained()).build();
    }
    ...
    private static DocumentRoutesDescriptorDto convert(DocumentRoutesDescriptor routes) {
        return DocumentRoutesDescriptorDto.newBuilder().setCurrentRoute(convert(routes.currentRoute())).setPreviousRoutes(convertRoutes(routes.previousRoutes())).build();
    }
    ...
    private static DocumentRouteDescriptorDto convert(DocumentRouteDescriptor route) {
        ...
        return DocumentRouteDescriptorDto.newBuilder().setRoutingKey(route.routingKey()).build();
    }
}
