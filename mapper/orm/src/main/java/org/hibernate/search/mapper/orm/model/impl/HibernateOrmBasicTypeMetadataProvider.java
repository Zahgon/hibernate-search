...
// Hibernate Commons annotations gives us wildcard types, we must make do.
...
public class HibernateOrmBasicTypeMetadataProvider {
    ...
    private static void collectPersistentClass(Builder metadataProviderBuilder, PersistentClass persistentClass) {
        ...
        if (persistentClass.hasPojoRepresentation()) {
            ...
            collectClassType(metadataProviderBuilder, javaClass, persistentClass.getIdentifierProperty(), persistentClass.getProperties().iterator());
        } else {
            collectDynamicMapType(metadataProviderBuilder, hibernateOrmEntityName, persistentClass.getSuperclass(), persistentClass.getIdentifierProperty(), persistentClass.getProperties().iterator());
        }
    }
    ...
    private static void collectClassProperty(Builder metadataProviderBuilder, Map<String, HibernateOrmBasicClassPropertyMetadata> collectedProperties, Class<?> propertyHolderJavaClass, Property property, boolean isId) {
        try {
    ...
    private static void collectDynamicMapProperty(Builder metadataProviderBuilder, Map<String, HibernateOrmBasicDynamicMapPropertyMetadata> collectedProperties, Property property) {
        // This also recurses and collects embedded types
        HibernateOrmTypeModelFactory<?> typeModelFactory = collectValue(metadataProviderBuilder, property.getValue());
    ...
    private static HibernateOrmTypeModelFactory<?> collectValue(Builder metadataProviderBuilder, Value value) {
        if (value instanceof Component) {
            ...
            return HibernateOrmTypeModelFactory.array(collectValue(metadataProviderBuilder, array.getElement()));
        } else if (value instanceof org.hibernate.mapping.Map) {
            org.hibernate.mapping.Map map = (org.hibernate.mapping.Map) value;
            return HibernateOrmTypeModelFactory.map((Class<? extends Map<?, ?>>) map.getCollectionType().getReturnedClass(), /*
					 ...
					 */
            collectValue(metadataProviderBuilder, map.getIndex()), collectValue(metadataProviderBuilder, map.getElement()));
            ...
            return HibernateOrmTypeModelFactory.collection((Class<? extends Collection<?>>) collection.getCollectionType().getReturnedClass(), collectValue(metadataProviderBuilder, collection.getElement()));
        } else if (value instanceof org.hibernate.mapping.ToOne) {
            org.hibernate.mapping.ToOne toOne = (org.hibernate.mapping.ToOne) value;
            return HibernateOrmTypeModelFactory.entityReference(toOne.getType().getReturnedClass(), toOne.getReferencedEntityName());
            ...
            return HibernateOrmTypeModelFactory.entityReference(oneToMany.getType().getReturnedClass(), oneToMany.getReferencedEntityName());
        } else {
    ...
    private static HibernateOrmTypeModelFactory<?> collectEmbedded(Builder metadataProviderBuilder, Component component) {
        if (component.isDynamic()) {
            ...
            if (!metadataProviderBuilder.dynamicMapTypeMetadata.containsKey(name)) {
                collectDynamicMapType(metadataProviderBuilder, name, null, /* No supertype */
                null, /* No ID */
            ...
            if (!metadataProviderBuilder.classTypeMetadata.containsKey(javaClass)) {
                collectClassType(metadataProviderBuilder, javaClass, null, /* No ID */
                component.getProperties().iterator());
            }
