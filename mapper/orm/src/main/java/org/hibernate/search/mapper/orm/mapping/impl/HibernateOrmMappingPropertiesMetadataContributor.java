    ...
    private void collectMetadataFromHibernateOrmMappingProperty(PojoAdditionalMetadataCollectorPropertyNode collector, Property property) {
        ...
        if (value instanceof org.hibernate.mapping.Collection) {
            ...
            if (referencedEntityName != null && mappedByPath != null && !mappedByPath.isEmpty()) {
                collector.value(getExtractorPath(collectionValue)).associationInverseSide(resolveMappedByPath(referencedEntityName, mappedByPath));
            }
            ...
            if (mappedByPath != null && !mappedByPath.isEmpty()) {
                collector.value(getExtractorPath(toOneValue)).associationInverseSide(resolveMappedByPath(referencedEntityName, mappedByPath));
            }
        } else if (value instanceof Component) {
            collector.value(getExtractorPath(value)).associationEmbedded();
