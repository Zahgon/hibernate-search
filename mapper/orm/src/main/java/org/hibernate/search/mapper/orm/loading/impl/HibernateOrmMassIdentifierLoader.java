...
public final class HibernateOrmMassIdentifierLoader<E, I> implements PojoMassIdentifierLoader {
    ...
    public HibernateOrmMassIdentifierLoader(HibernateOrmQueryLoader<E, I> typeQueryLoader, HibernateOrmMassLoadingContext options, PojoMassIdentifierSink<I> sink, SharedSessionContractImplementor session) {
        ...
        try {
            ...
            long totalCountFromQuery = typeQueryLoader.createCountQuery(session).setCacheable(false).uniqueResult();
            ...
            results = typeQueryLoader.createIdentifiersQuery(session).setCacheable(false).setFetchSize(options.idFetchSize()).scroll(ScrollMode.FORWARD_ONLY);
