...
public class EntityIdReader extends AbstractItemReader {
    ...
    private JobContextData getOrCreateJobContextData() {
        return JobContextUtil.getOrCreateData(jobContext, emfRegistry, entityManagerFactoryNamespace, entityManagerFactoryReference, serializedEntityTypes);
    }
    ...
    private class ChunkState implements AutoCloseable {
        ...
        private <E, I> ScrollableResults<I> createScroll(EntityTypeDescriptor<E, I> type, StatelessSession session) {
            ...
            if (lastCheckpointInfo != null) {
                conditions.add(type.idOrder().idGreater("HIBERNATE_SEARCH_LAST_CHECKPOINT_", lastCheckpointInfo.getLastProcessedEntityId()));
            ...
            return query.setReadOnly(true).setCacheable(false).setLockMode(LockModeType.NONE).setFetchSize(idFetchSize).scroll(ScrollMode.FORWARD_ONLY);
        }
    }
