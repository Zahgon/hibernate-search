...
public class StepProgressSetupListener extends AbstractStepListener {
    ...
    private static Long countAll(StatelessSession session, EntityTypeDescriptor<?, ?> type, ConditionalExpression reindexOnly) {
        return type.createCountQuery((SharedSessionContractImplementor) session, reindexOnly == null ? List.of() : List.of(reindexOnly)).setReadOnly(true).setCacheable(false).setLockMode(LockModeType.NONE).uniqueResult();
    }
}
